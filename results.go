...
// the `Result.Body`. This would be useful for Enterprise Cloud providers that have
// different fields in the response object than Enterprise Cloud proper.
func (r Result) ExtractInto(to interface{}) error {
	if r.Err != nil {
		log.Printf("[DEBUG] Response body: %s", r.Err)
		return r.Err
	}
	...
	log.Printf("[DEBUG] Response body: %s", b)
...
Enterprise Cloud, because most of the operations don't return response bodies,
but do have relevant information in headers.
*/
...
// ISO8601 describes a common time format used by some API responses.
// Expecially in storage SDP of Enterprise Cloud 2.0
const ISO8601 = "2006-01-02T15:04:05+0000"

type JSONISO8601 time.Time

func (jt *JSONISO8601) UnmarshalJSON(data []byte) error {
	// log.Printf("[DEBUG] ISO8601::UnmarshalJSON")
