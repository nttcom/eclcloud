...
func TestMaybeString(t *testing.T) {
	testString := ""
	var expected *string
	actual := eclcloud.MaybeString(testString)
...
func TestMaybeInt(t *testing.T) {
	testInt := 0
	var expected *int
	actual := eclcloud.MaybeInt(testInt)
	th.CheckDeepEquals(t, expected, actual)
...
func TestBuildQueryString(t *testing.T) {
	...
	actual, err := eclcloud.BuildQueryString(&opts)
	if err != nil {
	...
	_, err = eclcloud.BuildQueryString(&opts)
	if err == nil {
		t.Errorf("Expected error: 'Required field not set'")
	}
	th.CheckDeepEquals(t, expected, actual)

	_, err = eclcloud.BuildQueryString(map[string]interface{}{"Number": 4})
	if err == nil {
...
func TestBuildHeaders(t *testing.T) {
	...
	actual, err := eclcloud.BuildHeaders(&testStruct)
	th.CheckNoErr(t, err)
	th.CheckDeepEquals(t, expected, actual)

	testStruct.Num = 0
	_, err = eclcloud.BuildHeaders(&testStruct)
	if err == nil {
		t.Errorf("Expected error: 'Required header not set'")
	}

	_, err = eclcloud.BuildHeaders(map[string]interface{}{"Number": 4})
	if err == nil {
		t.Errorf("Expected error: 'Options type is not a struct'")
	}
}
...
func TestQueriesAreEscaped(t *testing.T) {
	...
	actual, err := eclcloud.BuildQueryString(foo{Name: "blah+?!!foo", Shape: "Triangl e"})
...
func TestBuildRequestBody(t *testing.T) {
	...
	// AuthOptions wraps a eclcloud AuthOptions in order to adhere to the AuthOptionsBuilder
	// interface.
	...
	for _, successCase := range successCases {
		actual, err := eclcloud.BuildRequestBody(successCase.opts, "auth")
		th.AssertNoErr(t, err)
	...
	for _, failCase := range failCases {
		_, err := eclcloud.BuildRequestBody(failCase.opts, "auth")
	...
	actual, err := eclcloud.BuildRequestBody(complexFields, "")
	th.AssertNoErr(t, err)
