...
type CreateOpts struct {
	...
	IPVersion eclcloud.IPVersion `json:"ip_version,omitempty"`
...
func (opts CreateOpts) ToSubnetCreateMap() (map[string]interface{}, error) {
	b, err := eclcloud.BuildRequestBody(opts, "subnet")
	if err != nil {
		return nil, err
	}
...
func IDFromName(client *eclcloud.ServiceClient, name string) (string, error) {
	...
	switch count {
	case 0:
		return "", eclcloud.ErrResourceNotFound{Name: name, ResourceType: "subnet"}
	case 1:
		return id, nil
	default:
		return "", eclcloud.ErrMultipleResourcesFound{Name: name, Count: count, ResourceType: "subnet"}
	}
}
