...
func (r *Object) UnmarshalJSON(b []byte) error {
	...
	*r = Object(s.tmp)
	...
	if s.LastModified != "" {
		t, err := time.Parse(eclcloud.RFC3339MilliNoZ, s.LastModified)
		if err != nil {
			t, err = time.Parse(eclcloud.RFC3339Milli, s.LastModified)
			if err != nil {
				return err
			}
		}
...
func (r *DownloadHeader) UnmarshalJSON(b []byte) error {
	...
	var s struct {
		...
		Date              eclcloud.JSONRFC1123 `json:"Date"`
		DeleteAt          eclcloud.JSONUnix    `json:"X-Delete-At"`
		LastModified      eclcloud.JSONRFC1123 `json:"Last-Modified"`
...
func (r *CopyHeader) UnmarshalJSON(b []byte) error {
	...
	var s struct {
		tmp
		ContentLength          string               `json:"Content-Length"`
		CopiedFromLastModified eclcloud.JSONRFC1123 `json:"X-Copied-From-Last-Modified"`
		Date                   eclcloud.JSONRFC1123 `json:"Date"`
		LastModified           eclcloud.JSONRFC1123 `json:"Last-Modified"`
	}
	err := json.Unmarshal(b, &s)
