...
import (
	...
	"crypto/sha1"
	"fmt"
	"github.com/nttcom/eclcloud/ecl/objectstorage/v1/accounts"
...
// respectively.
func List(c *eclcloud.ServiceClient, containerName string, opts ListOptsBuilder) pagination.Pager {
...
func (opts DownloadOpts) ToObjectDownloadParams() (map[string]string, string, error) {
	...
	h, err := eclcloud.BuildHeaders(opts)
	if err != nil {
...
func Download(c *eclcloud.ServiceClient, containerName, objectName string, opts DownloadOptsBuilder) (r DownloadResult) {
	...
	resp, err := c.Get(url, nil, &eclcloud.RequestOpts{
		MoreHeaders: h,
...
// of 3 times.
func Create(c *eclcloud.ServiceClient, containerName, objectName string, opts CreateOptsBuilder) (r CreateResult) {
	...
	resp, err := c.Put(url, nil, nil, &eclcloud.RequestOpts{
		RawBody:     b,
		MoreHeaders: h,
	})
	r.Err = err
...
func Copy(c *eclcloud.ServiceClient, containerName, objectName string, opts CopyOptsBuilder) (r CopyResult) {
	h := make(map[string]string)
	...
	resp, err := c.Request("COPY", url, &eclcloud.RequestOpts{
		MoreHeaders: h,
		OkCodes:     []int{201},
	})
	if resp != nil {
...
func Delete(c *eclcloud.ServiceClient, containerName, objectName string, opts DeleteOptsBuilder) (r DeleteResult) {
...
// function.
func Get(c *eclcloud.ServiceClient, containerName, objectName string, opts GetOptsBuilder) (r GetResult) {
	...
	resp, err := c.Head(url, &eclcloud.RequestOpts{
		MoreHeaders: h,
		OkCodes:     []int{200, 204},
	})
	if resp != nil {
...
func Update(c *eclcloud.ServiceClient, containerName, objectName string, opts UpdateOptsBuilder) (r UpdateResult) {
	h := make(map[string]string)
	if opts != nil {
	...
	resp, err := c.Post(url, nil, nil, &eclcloud.RequestOpts{
		MoreHeaders: h,
	})
	if resp != nil {
		r.Header = resp.Header
...
type CreateTempURLOpts struct {
	...
	// empty, the default Enterprise Cloud URL split point will be used ("/v1/").
	Split string
}
...
func CreateTempURL(c *eclcloud.ServiceClient, containerName, objectName string, opts CreateTempURLOpts) (string, error) {
	if opts.Split == "" {
		opts.Split = "/v1/"
	}
