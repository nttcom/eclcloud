...
func Get(c *eclcloud.ServiceClient, opts GetOptsBuilder) (r GetResult) {
	...
	resp, err := c.Head(getURL(c), &eclcloud.RequestOpts{
		MoreHeaders: h,
		OkCodes:     []int{204},
	})
	if resp != nil {
...
func (opts UpdateOpts) ToAccountUpdateMap() (map[string]string, error) {
	headers, err := eclcloud.BuildHeaders(opts)
	if err != nil {
		return nil, err
	}
...
func Update(c *eclcloud.ServiceClient, opts UpdateOptsBuilder) (r UpdateResult) {
	h := make(map[string]string)
	if opts != nil {
	...
	resp, err := c.Request("POST", updateURL(c), &eclcloud.RequestOpts{
		MoreHeaders: h,
