...
func Create(c *eclcloud.ServiceClient, containerName string, opts CreateOptsBuilder) (r CreateResult) {
	...
	resp, err := c.Request("PUT", createURL(c, containerName), &eclcloud.RequestOpts{
		MoreHeaders: h,
...
func Delete(c *eclcloud.ServiceClient, containerName string) (r DeleteResult) {
...
// metadata.
func Update(c *eclcloud.ServiceClient, containerName string, opts UpdateOptsBuilder) (r UpdateResult) {
	h := make(map[string]string)
	if opts != nil {
	...
	resp, err := c.Request("POST", updateURL(c, containerName), &eclcloud.RequestOpts{
		MoreHeaders: h,
		OkCodes:     []int{201, 202, 204},
	})
	if resp != nil {
...
// function.
func Get(c *eclcloud.ServiceClient, containerName string, opts GetOptsBuilder) (r GetResult) {
	h := make(map[string]string)
	if opts != nil {
	...
	resp, err := c.Head(getURL(c, containerName), &eclcloud.RequestOpts{
		MoreHeaders: h,
		OkCodes:     []int{200, 204},
	})
	if resp != nil {
