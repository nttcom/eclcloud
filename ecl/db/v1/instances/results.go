...
import (
	...
	"github.com/nttcom/eclcloud/ecl/db/v1/datastores"
...
func (r *Fault) UnmarshalJSON(b []byte) error {
	...
	var s struct {
		tmp
		Created eclcloud.JSONRFC3339NoZ `json:"created"`
	}
...
func (r *Instance) UnmarshalJSON(b []byte) error {
	...
	var s struct {
		tmp
		Created eclcloud.JSONRFC3339NoZ `json:"created"`
		Updated eclcloud.JSONRFC3339NoZ `json:"updated"`
	}
	err := json.Unmarshal(b, &s)
	if err != nil {
		return err
...
func (page InstancePage) NextPageURL() (string, error) {
	var s struct {
		Links []eclcloud.Link `json:"instances_links"`
	}
