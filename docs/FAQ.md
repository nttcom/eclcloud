...
`eclcloud.ProviderClient.HTTPClient` field):

```go
//...

// LogRoundTripper satisfies the http.RoundTripper interface and is used to
// customize the default Eclcloud RoundTripper to allow for logging.
...
Enterprise Cloud request/response objects may differ among variable names or types.
...
Some Enterprise Cloud services have extensions.
Extensions that are supported in Eclcloud can be combined to create a custom object:
...
func (r *MyVolume) UnmarshalJSON(b []byte) error {
	...
	var s struct {
		tmp
		TimeCreated eclcloud.JSONRFC3339MilliNoZ `json:"created_at"`
	}
