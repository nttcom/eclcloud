...
- [x] Have an Enterprise Cloud environment available to test with
...
We encourage you to browse the existing Eclcloud code to find examples
...
If you're adding a single field, then a single Pull Request is also fine.
...
Single API calls can also be submitted as a single Pull Request.
...
### Adding an Entire Enterprise Cloud Project

To add an entire Enterpsei Cloud project, you must break each set of API calls into
...
generate your own fixtures using the Enterprise Cloud environment you're
[testing](step-04-acceptance-testing.md) with.

### Documentation

All documentation in Eclcloud is done through in-line `godoc`. Please make
...
Once you have your code written, submit a Pull Request to Eclcloud and
proceed to [Step 5](step-05-code-review.md).
