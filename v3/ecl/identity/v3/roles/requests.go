...
func (opts CreateOpts) ToRoleCreateMap() (map[string]interface{}, error) {
	b, err := eclcloud.BuildRequestBody(opts, "role")
	if err != nil {
		return nil, err
	}
...
func Update(client *eclcloud.ServiceClient, roleID string, opts UpdateOptsBuilder) (r UpdateResult) {
	b, err := opts.ToRoleUpdateMap()
	if err != nil {
		r.Err = err
		return
	}
	_, r.Err = client.Patch(updateURL(client, roleID), &b, &r.Body, &eclcloud.RequestOpts{
		OkCodes: []int{200},
	})
	return
}
...
func Delete(client *eclcloud.ServiceClient, roleID string) (r DeleteResult) {
...
func ListAssignments(client *eclcloud.ServiceClient, opts ListAssignmentsOptsBuilder) pagination.Pager {
...
func ListAssignmentsOnResource(client *eclcloud.ServiceClient, opts ListAssignmentsOnResourceOpts) pagination.Pager {
	// Check xor conditions
	_, err := eclcloud.BuildRequestBody(opts, "")
	if err != nil {
		return pagination.Pager{Err: err}
	}
...
func Assign(client *eclcloud.ServiceClient, roleID string, opts AssignOpts) (r AssignmentResult) {
	// Check xor conditions
	_, err := eclcloud.BuildRequestBody(opts, "")
	if err != nil {
		r.Err = err
	...
	_, r.Err = client.Put(assignURL(client, targetType, targetID, actorType, actorID, roleID), nil, nil, &eclcloud.RequestOpts{
		OkCodes: []int{204},
	})
	return
}
...
func Unassign(client *eclcloud.ServiceClient, roleID string, opts UnassignOpts) (r UnassignmentResult) {
	// Check xor conditions
	_, err := eclcloud.BuildRequestBody(opts, "")
	if err != nil {
		r.Err = err
	...
	_, r.Err = client.Delete(assignURL(client, targetType, targetID, actorType, actorID, roleID), &eclcloud.RequestOpts{
		OkCodes: []int{204},
	})
	return
}
